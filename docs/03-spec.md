...
- **Catálogo** `/tarjetas` (#70, grill del 2026-09-25): todas las familias con foto, banco, costo anual y beneficios vigentes, para "quiero sacarme una tarjeta" (distinto de `/comparar`, que es "cuál de las mías uso hoy"). Orden por defecto = beneficios vigentes que aplican (los "para todas las tarjetas del banco" cuentan para cada una; se muestran aparte los "solo con esta"), sin podio numerado; selector por menor costo o menor ingreso mínimo, sin dato al fondo. Filtros en query (`?banco=&tipo=&red=&costo=&ingreso=&programa=`), canonical a `/tarjetas`. Programa millas/puntos clasificado por palabras clave sobre el texto de la ficha. No entra en la barra inferior: sección en la home y link desde `/banco/[id]`.
